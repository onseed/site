...
$.fn.extend({//#################### 3.uxLayerPop ####################
	uxLayerPop : function(opt) {
		...
		return this.each(function(){
			...
			/* 스크롤을 rock하면 레이어팝업내 스크롤도 rock돼서 스크롤 처리하는것을 추천 20211201
			...
			try {
				window.addEventListner("test", null, object.defineProperty({}, 'passive', {
			...
			*/
			...
			function disableScroll() {//console.log("스크롤 막기");
				...
				window.addEventListener(wheelEvent, preventDefault, wheelOpt); // moder desktop
			...
			function enableScroll() {//console.log("스크롤 풀기");				
				...
				window.removeEventListener(wheelEvent, preventDefault, wheelOpt); // moder desktop
				window.removeEventListener('touchmove', preventDefault, wheelOpt); // mobile
				window.removeEventListener('keydown', preventDefaultForScrollKeys, false);
				window.removeEventListener('wheel', preventDefault, {passive:false}); //modern standard 20190917 chrome 패치
				*/			
			}
			...
			var app={
				init: function(){
					...
					if ($vscrollbar.size() === 0) {// 스크롤바가 없으면 자동 생성해 준다.
						...
						//$scroller.removeClass("use_scroll"); //스크롤이 생길때
					}
    ...
    pageAnchors : function(opt){
        ...
        return this.each(function(){
            var $this = $(this),
                ...
                eff = opt.eff,
                target = opt.target,
                targetFocus = opt.targetFocus,
                thisHref = $this.attr('href'),
                targetOffset = $('html').offset(),
                targetTop = targetOffset.top - addTop,
                place = $('html, body');

            if (target !== null) {
            	targetOffset = $(target).offset();
                targetTop = targetOffset.top - addTop; //console.log(targetTop, addTop);
            }
...
$.fn.extend({//#################### 101.tapList ####################
	tapList : function(opt){
	    var defaults = {},
	    	opt = $.extend(defaults,opt);
	    return this.each(function(){//return
	        ...
	        $this.find("a").attr("aria-selected","false").attr("role","tab");
	        $this.next(".tab_cont").find("section").attr('tabindex', 0).attr("role","tabpanel");
	        
	    	if($this.find('.on').length){//탭리스트에 on클래스("a")가 있을때        		
  	        	$a.attr("aria-selected","false").attr('tabindex', -1);	    		
	    		$this.find('.on').closest("li").addClass("on");
	    		$this.find('a.on').attr("aria-selected","true").attr('tabindex', 0);
	    		$("#"+$this.find('a.on').attr("aria-controls")).css("display","block");
	    	}
        			
        	$this.find('a').on("keyup",function(e){//key 다운시
        		if(e.key == "ArrowLeft" || e.key == "ArrowRight"){
	        		e.preventDefault();
	        		var currentIdx  = $(this).parent("li").index();
	        		var tabLeng = $(this).closest(".tab_list").find("li").length;	        		
	        		var txrgatIdx = 0;
	        		
	        		if(e.key == "ArrowLeft"){
	        			txrgatIdx =  currentIdx - 1;
	        		}else if(e.key == "ArrowRight"){
	        			txrgatIdx =  currentIdx + 1;
	        		}
        			if(txrgatIdx <= -1 || txrgatIdx >= tabLeng){ //-1이 되거나 //leng을 넘거나
        				txrgatIdx = 0;	
        			}//console.log("txrgatIdx: "+txrgatIdx + " / tabLeng: "+ tabLeng);
   	        		$li.removeClass("on");
   	        		$a.removeClass("on").attr("aria-selected","false").attr('tabindex', -1);
   	        		$this.find("li").eq(txrgatIdx).addClass("on");	        				
   	        		$this.find("li").eq(txrgatIdx).find("a").attr("aria-selected","true").addClass("on").attr('tabindex', 0).focus();
   	        		$this.closest(".tab_list").next(".tab_cont").find("section").css("display","none");
   	        		$this.closest(".tab_list").next(".tab_cont").find("section").eq(txrgatIdx).css("display","block");
        		}
        	});
        	$this.find('a').on("click",function(e){
        		...
        		$a.removeClass("on").attr("aria-selected","false");	        		
        		$(this).addClass("on").attr("aria-selected","true");        		
        		var idx  = $(this).parent("li").index();
        		$(this).closest(".tab_list").next(".tab_cont").find("section").css("display","none");
        		$(this).closest(".tab_list").next(".tab_cont").find("section").eq(idx).css("display","block");        			        		
        	});
	    });//return.end
	}
});//#################### 101.tapList.end ####################

$.fn.extend({//#################### 102.linkList ####################
	linkList : function() {
	    var $this = $(this);	
	    $this.find("li").on("mouseover", function(){
	    	$this.find("li").removeClass("over");
	    	$this.find("li").removeClass("next");	
	    	$(this).prev("li").addClass("next");
	    	$(this).addClass("over");
	    });
	...
	pagination : function(){
		var $this = $(this);
		$this.attr("aria-label","페이지 네비게이션");
		$this.find("a").each(function(){
			$(this).attr("aria-label", $(this).text() +"페이지 이동");
		});
		$this.find(".on").attr("aria-current","true").attr("aria-label","현재 페이지");
	}
});//#################### 102.linkList ####################
...
var uxer = {//공통 처리
	'init' : function() {
		...
		$(".tab_list").tapList();	//탭 리스트
		$(".link_list").linkList(); //링크 리스트
		$(".paginate").pagination(); //페이지네이션
		
		$('.acco_box.type1').uxAccodian(); //토글 기본형
		$('.acco_box.type2').uxAccodian({toggleCont : true}); //토글 QnA형
		$('.acco_box.checkbox').uxAccodian({inputCheck : true}); //토글 QnA형
		$('.ui_scrollview').uxScrollView(); //스크롤뷰		
