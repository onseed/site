...
$(document).ready(function(){
	...
	$.when(svg_front_load, svg_side_load, svg_back_load, svg_top_load).then(
		function(data1, data2, data3, data4){//load success
			...
			$("#sort_color_tab .tab01").css("backgroundImage","url("+img_url+"color/amy_color_small0.jpg)");
...
function bag_color_Img_url_make(num){//color 처리 ev
	if(custom_status == "color_base"){
		...
		$("#sort_color_tab .tab01, #sort_color_tab .tab03").css("backgroundImage","url("+img_url+"color/amy_color_small"+bag_base+".jpg)");
		if(bag_base == 0){
		...
		$("#sort_color_tab .tab02").css("backgroundImage","url("+img_url+"color/amy_color_small"+bag_side+".jpg)");
		...
		bag_front = num; //컬러값 front 선택
		$("#sort_color_tab .tab03").css("backgroundImage","url("+img_url+"color/amy_color_small"+bag_front+".jpg)");
		if(bag_front == 0){
			$("#sort_color_tab .tab03").removeClass("on");
		}else{
			$("#sort_color_tab .tab03").addClass("on");
		...
		$("#sort_color_tab .tab04").css("backgroundImage","url("+img_url+"color/amy_color_small"+bag_handle+".jpg)");
		if(bag_handle == 0){
			$("#sort_color_tab .tab04").removeClass("on");
		}else{
			$("#sort_color_tab .tab04").addClass("on");
...
function load_skin(){//##커스텀 스킨 로드!
	...
	$("#unit01_5, #unit01_3, #unit01_4").attr("fill","url(#svg_front_hadle)");//손잡이
	...
	$("#unit02_3").attr("fill","url(#svg_side_back)");//몸통 뒷판
	...
	$("#unit02_2").attr("fill","url(#svg_side_strab)");//스트랩
	...
	$("#unit02_7, #unit02_8, #unit02_9, #unit02_10, #unit02_11").attr("fill","url(#svg_side_emblem)");//엠블렘, 쟈크, 스트랩 장식1, 왼쪽받침, 오른쪽받칩
	...
	$("#unit02_4, #unit02_5, #unit02_6").attr("fill","url(#svg_side_hadle)");//손잡이
	...
	$("#unit03_6, #unit03_7, #unit03_8, #unit03_9, #unit03_10").attr("fill","url(#svg_back_emblem)");//엠블렘, 쟈크, 스트랩 장식1, 왼쪽받침, 오른쪽받칩
	...
	$("#unit03_3, #unit03_4, #unit03_5").attr("fill","url(#svg_back_hadle)");//손잡이

	/*top*/
	//top_bag_strab_url = base_url+bag_type+"_top_base"+ bag_base+"_"+bag_front_pattern+".jpg"; //bag_front(내피는 패턴영향을 받지 않음)
	top_bag_strab_url = base_url+bag_type+"_top_base"+ bag_base+"_0.jpg"; //bag_front(top은 패턴영향이 없음)
	...
	$("#unit04_4, #unit04_5, #unit04_6, #unit04_7, #unit04_8, #unit04_9, #unit04_10, #unit04_11").attr("fill","url(#svg_top_emblem)");//자크, 위쪽 단추, 아래쪽 단추

	//top_bag_handle_url = base_url+bag_type+"_top_base"+ bag_handle +"_"+bag_front_pattern +".jpg";//bag_handl
	top_bag_handle_url = base_url+bag_type+"_top_base"+ bag_handle +"_0.jpg";//bag_handl(top은 패턴영향이 없음)
	$("#svg_top_hadle_img").attr("xlink:href", top_bag_handle_url);
	$("#unit04_2, #unit04_3").attr("fill","url(#svg_top_hadle)");//위쪽 손잡이, 아래쪽 손잡이
