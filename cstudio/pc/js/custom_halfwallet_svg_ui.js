...
$(document).ready(function(){
	...
	$.when(svg_front_load, svg_side_load, svg_back_load, svg_top_load).then(
		function(data1, data2, data3, data4){//load success
			...
			$("#sort_color_tab li").css("backgroundImage","url("+img_url+"color/wallet_color_small0.jpg)");
...
function bag_color_Img_url_make(num){//color 처리 ev
	if(custom_status == "color_base"){
		...
		//bag_side = num; 컬러값 side 선택 20151015 선택제외 할것
		//bag_handle = num; //컬러값 handle 선택

		$("#sort_color_tab .tab01").css("backgroundImage","url("+img_url+"color/halfwallet_color_small"+bag_base+".jpg)"); //20160126 추가
		if(bag_base == 0){
		...
		$("#sort_color_tab .tab02").css("backgroundImage","url("+img_url+"color/halfwallet_color_small"+bag_side+".jpg)");//20160126 추가
...
function load_skin(){//##커스텀 스킨 로드 20150915 수정처리
	...
	$("#unit01_2").attr("fill","url(#svg_front_emblem)");//엠블렘
	...
	$("#unit02_2").attr("fill","url(#svg_side_side)");//사이드를 이너포켓으로 대체
	...
	$("#unit02_3").attr("fill","url(#svg_side_emblem)");//엠블렘
