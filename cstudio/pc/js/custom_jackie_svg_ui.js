...
function bag_color_Img_url_make(num){//color 처리 ev
	if(custom_status == "color_base"){
		...
		$("#sort_color_tab .tab01").css("backgroundImage","url("+img_url+"color/jackie_color_small"+bag_base+".jpg)"); //20150918 추가
		if(bag_base == 0){
			$("#sort_color_tab .tab01").removeClass("on"); //20150918 추가
			$("#sort_color_tab .tab01").addClass("on");		
		}else{
			$("#sort_color_tab .tab01").addClass("on"); //20150918 추가
		}
	}else if(custom_status == "color_side"){
		bag_side = num; //컬러값 side 선택
		$("#sort_color_tab .tab02").css("backgroundImage","url("+img_url+"color/jackie_color_small"+bag_side+".jpg)");
...
function load_skin(){//##커스텀 스킨 로드 20150915 수정처리
	...
	$("#unit01_3,#unit01_4").attr("fill","url(#svg_front_emblem)");//엠블렘, 쟈크, 스트랩 장식1, 스트랩 장식2, 왼쪽받침, 오른쪽받칩
	...
	side_bag_side_url = base_url+bag_type+"_side_base"+ bag_side +"_"+bag_side_pattern +".jpg";//bag_handl
	$("#svg_side_side_img").attr("xlink:href", side_bag_side_url);
	$("#unit02_3").attr("fill","url(#svg_side_side)");//사이드를 이너포켓으로 대체
	...
	$("#unit03_3").attr("fill","url(#svg_back_emblem)");//스트랩 장식


	/*top:_open으로 대체*/
	top_bag_front_url = base_url+bag_type+"_open_base"+ bag_front+"_0.jpg"; //bag_front: 프론트는 패턴영향없음
	$("#svg_top_strab_img").attr("xlink:href", top_bag_front_url);
	$("#unit04_1").attr("fill","url(#svg_top_strab)");//몸통

	top_emblem_url = base_url+bag_type+"_open_emblem"+ bag_emblem +".jpg";//bag_emblem
	$("#svg_top_emblem_img").attr("xlink:href", top_emblem_url);
	$("#unit04_3, #unit04_4, #unit04_5, #unit04_6, #unit04_7").attr("fill","url(#svg_top_emblem)");//자크1, 자크2, 단추, 스트랩장식1, 스트랩장식2

	top_bag_side_url = base_url+bag_type+"_open_base"+ bag_side +"_"+bag_side_pattern +".jpg";//bag_handl
...
$(document).ready(function(){
	$("#section_color_base,#btn_color_base_set ul button, #section_pattern_body, #btn_pattern_body_set ul button").click(function(){//아웃사이드 탭
		if (view_status=='top'){
			view_status="top";
			$("#front_view").click();
		}
