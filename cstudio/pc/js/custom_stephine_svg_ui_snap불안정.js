...
var side_more_ani; //좌측 more ani 20111013
...
$(window).load(function(){
	...
	function side_menu_loaded () {//사이드 메뉴 스크롤 20151113 수정
		myScroll = new IScroll('#side_menu_wrap', { scrollX: false, scrollY: true,/* eventPassthrough: true, preventDefault: false, hideScrollbar: false,*/bounce: false, scrollbarClass: 'myScrollbar',  mouseWheel: true, scrollbars: true, click: true//, snap : '.menu_section'
	});	

	var menu_section1  = $('.color_choice').position().top;
	var menu_section2  = $('.pattern_choice').position().top - 50;
	var menu_section3  = $('.emblem_choice').position().top -99 ;
	var menu_section4  = $('.strab_choice').position().top - 148;
	var menu_section5  = $('.wappen_choice').position().top -197;

	myScroll.on('scrollEnd',function(){
		var myScroll_y = myScroll.y;
		console.log(myScroll_y);
		console.log("-" + menu_section1 + " : " + "-" + menu_section2 + " : "  + "-" + menu_section3 + " : "  + "-" + menu_section4 + " : "  + "-" + menu_section5);

		if(myScroll_y >= -menu_section2){//컬러탭영역일때
			console.log("컬러탭");
			$(".tab_color a").click();
		}else	if(myScroll_y < -menu_section2 && myScroll_y >= -menu_section3){//프린트탭영역일때
			console.log("프린트탭");
			$(".tab_print a").click();
		}else	if(myScroll_y < -menu_section3 && myScroll_y >= -menu_section4){//엠블램영역일때
			console.log("엠블렘탭");
			$(".tab_emblem a").click();
		}else	if(myScroll_y < -menu_section4 && myScroll_y >= -menu_section5){//스트랩영역일때
			console.log("스트랩탭");
			$(".tab_strap a").click();
		}
		else{
	...
	$("#side_menu_wrap").mouseover(function(){//side more ani
		$('.more_div').fadeOut("fast"); //201511113 수정
		window.clearInterval(side_more_ani);
	});
...
function svg_mouse_ev(){
    ...
    var top  = $('.pattern_choice').position().top - 48;
		...
		//myScroll.scrollToElement(".pattern_choice", "500");
		tab_option_init();
		$(this).parent().addClass("on");
		return false;
	});
    ...
    var top  = $('.emblem_choice').position().top - 97;
		myScroll.scrollBy(0, -top,  "300");
		//myScroll.scrollToElement(".emblem_choice", "500");
		tab_option_init();
		$(this).parent().addClass("on");
		return false;
	});
    ...
    var top  = $('.strab_choice').position().top - 146;
		myScroll.scrollBy(0, -top,  "300");
		//myScroll.scrollToElement(".strab_choice", "500");
		tab_option_init();
		$(this).parent().addClass("on");
		return false;
	});
    ...
    var top  = $('.wappen_choice').position().top - 195;
		myScroll.scrollBy(0, -top,  "300");
		//myScroll.scrollToElement(".wappen_choice", "500");
		tab_option_init();
		$(this).parent().addClass("on");
		return false;
	});
...
function tab_option_init(){		
	...
	$(".wappen_module").css("display","none");//wappen_module
	$('.more_div').fadeOut("fast");// 좌측 more 처리 20111113 
	window.clearInterval(side_more_ani);// 좌측 more 처리 20111113 
	wappen_grid = false;
}
//## end. svg 오버,클릭처리를 탭바로가기로 변경 20151027 ##//
...
function glimme_popup_open(){//glimme_popup_open 20151113 추가
	$('#glime_detail, .layer_bg').fadeIn("fast",function(){
		$('#movPlayer').attr("src","http://www.youtube.com/embed/zXRRArarjVc?autoplay=1&amp;theme=light&amp;controls=0&amp;showinfo=0&amp;wmode=opaque");
		pop_vertical();
	});	
}
//*************팝업 끝
